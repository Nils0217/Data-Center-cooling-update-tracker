...
4. **Energy Inefficiency & Dynamic Tariffs**: Legacy HVAC chillers running with poor power factors ($\text{PF} < 0.85$) waste massive amounts of power, which must be converted into financial impact based on real-time grid rates.
...
* **Green Zone Highlighting**: Outlines the ultra-efficient liquid cooling zone ($\text{PUE} \le 1.20, \text{WUE} \le 0.10$).
...
* **Low Power Factor Penalty Table**: Identifies chillers running with $\text{PF} < 0.85$ and calculates annual financial loss:
